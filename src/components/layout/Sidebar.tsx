...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  ...
  const menuItems = [
    { id: 'dashboard' as ActiveTab, label: currentUser.role === 'student' ? 'Meu Painel' : 'Dashboard', icon: LayoutDashboard },
    { id: 'delegations' as ActiveTab, label: currentUser.role === 'student' ? 'Salas & Bancadas' : 'Salas & Turmas', icon: Users },
    { id: 'committees' as ActiveTab, label: 'Comitês', icon: Building2 },
    { id: 'resolutions' as ActiveTab, label: currentUser.role === 'student' ? 'Minutas & Pautas' : 'Resoluções', icon: FileText },
          ...
          {timerMode === 'reuniao' ? (
            ...
            <div>
              ...
              {currentUser.role === 'admin' ? (
                ...
                <div className="text-[10px] text-center font-bold text-slate-400 pt-1 border-t border-slate-200/50">
                  Tempo Oficial da Assembleia
                </div>
              )}
            </div>
          ) : (
            ...
            <div>
              ...
              {/* Presets e Controles da Deliberação */}
              {currentUser.role === 'admin' ? (
                <>
